...
export function parsedItemToMRZResult(parsedItem: ParsedResultItem): MRZResult | undefined {
  ...
  mrzResult.documentNumber = parsedItem.parsedFields.passportNumber?.value || parsedItem.parsedFields.documentNumber?.value;
  ...
  let birthMonthStr = `${birthMonth && birthMonth < 10 ? '0' : ''}${birthMonth}`;
  let birthDayStr = `${birthDay && birthDay < 10 ? '0' : ''}${birthDay}`;
